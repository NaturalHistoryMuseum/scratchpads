<?php
/**
 * $Id: TpSchemaInspector.php 417 2007-08-06 21:31:04Z rdg $
 * 
 * LICENSE INFORMATION
 * 
                ...
                if ( $expression->GetType() == EXP_CONCEPT )
                {
                    ...
                    if ( is_object( $concept ) and $concept->IsMapped() ) 
                    {
                        ...
                        if ( strcasecmp( $mapping_type, 'SingleColumnMapping' ) == 0 and 
                             ! TpServiceUtils::Contains( $path, $this->mIndexingElement ) )
                        {
                            $msg = 'Node "'.$path.'" has an incompatible local '.
                                   'mapping type (single column mapping) for being '.
                                   'outside the indexingElement';
