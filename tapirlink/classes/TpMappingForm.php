<?php
/**
 * $Id: TpMappingForm.php 434 2007-09-09 23:50:56Z rdg $
 * 
 * LICENSE INFORMATION
 * 
...
class TpMappingForm extends TpWizardForm
{
        ...
        if ( isset( $_REQUEST['load_schemas'] ) ) 
        {
                 ...
                 strlen( $_REQUEST['load_from_location'] ) > 0 )
            {
                     ...
                     strlen( $_REQUEST['handler'] ) > 0 )
                {
                    ...
                    $r_local_mapping->LoadNewSchema( $location, $handler );
                }
                else
                {
    ...
    {
        // Note: This method assumes that _LoadIndexOfMappingReferences
        //       and _LoadReferences on the respective file were already called

        // Check if there's an equivalent mapping that can be reused

        $concept_id = $concept->GetId();

        if ( is_array( $this->mConceptsWithReferences ) and
             isset( $this->mConceptsWithReferences[$concept_id] ) )
        {
            $references = $this->mConceptsWithReferences[$concept_id];

            $local_mapping = $this->mResource->GetLocalMapping(); // work on a copy

            foreach ( $references as $reference ) // $reference is a concept id
            {
                $equivalent_concept = $local_mapping->GetConcept( $reference );
