				...
				.each(
						function(i) {
							...
							if (!columnDef['states'][values[i]]) {
								...
								error = "'"
										+ values[i]
										+ "' is not an option for this controlled state. Please select from: <br />";

								for ( var j in columnDef['states']) {

									error += '<strong>' + j + "</strong> "
											+ columnDef['states'][j]['state']
											+ "<br />";
								...
								return;
									...
									&& (values[i].indexOf('?') != -1 || values[i]
											.indexOf('-') != -1)) {

								valid = false;

								error = 'Sorry, you cannot select both a state and '
										+ values[i] + '.';
...
function nexusNumericStateValidator(value, columnDef, $container) {
	...
	if (value.length) {
		...
		if (value.indexOf('-') > 0) { // User has entered a range so loop
			// through checking both are numeric
			var numeric_values = value.split('-');

			for (i in numeric_values) {

				if (!isNumeric(numeric_values[i])) {
					valid = false;
					nexusErrorBeautyTip(
							$container,
							numeric_values[i]
									+ ' is not a numeric value - please enter a range of numbers.');
					break;
		...
		} else if (!isNumeric(value)) { // User has entered numeric (including
			// minus numbers)
			valid = false;
...
function nexusDNAStateValidator(value, columnDef, $container) {
	...
	if (value.length && (value.toString().match(/^[ACGTUYRKW\-\?]*$/i) == null)) {
		...
		nexusErrorBeautyTip($container,
				'Please enter only DNA sequence characters (ACGTUYRKW) or ? and -')
...
var nexusFreeEntryUpdateItem = function(value, columnDef, item) {
	...
	if (intersection_info[character_tid]
			&& intersection_info[character_tid][taxa_tid]
			&& intersection_info[character_tid][taxa_tid]['state_nid']) {
