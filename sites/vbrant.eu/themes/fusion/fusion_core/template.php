<?php
// $Id: template.php,v 1.1.2.3 2009/09/29 07:04:15 sociotech Exp $
 ...
 * Theme preprocessing
 */
function fusion_core_preprocess(&$vars) {
  global $user;                                           // Current user
  $vars['logged_in'] = ($user->uid > 0) ? TRUE : FALSE;   // Logged in
}
...
function fusion_core_preprocess_page(&$vars) {
  ...
  $body_classes = array();
  $body_classes[] = ($vars['logged_in']) ? 'logged-in' : 'not-logged-in';                            // User logged in
  $body_classes[] = ($vars['is_front']) ? 'front' : 'not-front';                                     // Front page
  ...
  if (isset($vars['node'])) {
    ...
    $body_classes[] = ($vars['node']->type) ? 'node-type-'. $vars['node']->type : '';                // Node type
  }
  else {
  ...
  $body_classes[] = (theme_get_setting('theme_font_size') != 'none') ? theme_get_setting('theme_font_size') : '';              // Font size
...
function fusion_core_preprocess_block(&$vars) {
  ...
  if (isset($vars['skinr']) && strpos($vars['skinr'], 'superfish') && 
