...
function initMatrixEditor() {
	...
	if ($.browser.msie && $.browser.version < 7) {
		$("#myGrid").html(
				'Sorry, this feature isn\'t currently available for IE6');
		return;
	}
	...
	grid.onColumnsResized = function(e, ui) {
		...
		if (ui.originalSize.width != $(ui.element).width()) {
			...
			$.post(
					Drupal.settings.matrixEditorCallbackPath
							+ '/column_resized', args);
	...
	grid.onColumnsReordered = function(e, ui) {
		...
		$.post(Drupal.settings.matrixEditorCallbackPath + '/reorder_columns',
				args);
	...
	$('#matrix-editor-panel input.form-submit').click(
			function() {

				$loader = $(
						"<img class='imageLoader' src='/"
								+ Drupal.settings.matrixEditorPath
								+ "/images/ajax-loader.gif' />").insertAfter(
						$(this));
				...
				if ($('#matrix-editor-field iframe').length) {
					...
					args += '&body='
							+ $('#matrix-editor-field iframe').contents().find(
									'body').html();
				...
				$.post(Drupal.settings.matrixEditorCallbackPath
						+ '/update_node', args, function(response) {

					var cell = $('#myGrid .editor-text').parents('div.c').attr(
							'cell');
...
var MatrixCellEditor = function($container, columnDef, value, dataContext) {
    ...
    objHttpDataRequest.abort();
		...
		field_args = columnDef.args + '&field=' + columnDef.id + '&nid='
				+ dataContext.id + '&view_field=' + columnDef.field;

		$('#matrix-editor-field').html(
	...
	this.isValueChanged = function() {
		return (!($input.html() == "" && defaultValue == null))
				&& ($input.html() != defaultValue);
	}
