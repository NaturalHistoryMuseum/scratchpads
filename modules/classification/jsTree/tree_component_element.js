...
***********************************************************************/
			...
			checked    : {
				data   : false,         // FALSE or STRING - comma separated nodes that need to be checked
				url    : false          // FALSE or STRING - url to JSON document to retrieve what nodes need to be checked
			},
/***********************************************************************/
...
***********************************************************************/
			if(opts && opts.checked) {
				this.settings.checked = $.extend({}, this.settings.checked,opts.checked);
				delete opts.checked;
			}
/***********************************************************************/
		...
		refresh : function (obj) {
			...
			this.checked = Array();
/***********************************************************************
	Added by D. Shorthouse, cookie check removed
***********************************************************************/
            if(this.settings.ancestry) {
				var str = this.settings.ancestry;
			...
			if(this.settings.checked.url && this.settings.ancestry) {
			   var _this = this;
	           $.getJSON(this.settings.checked.url, function (data) {
                 if(data.length) {
                   for(var i = 0; i < data.length; i++) {
	                  _this.checked.push(data[i].id);
                   }
                 }
               });
            }
            if(this.settings.checked.data && this.settings.ancestry) {
	           var str = this.settings.checked.data;
			   if (str.constructor.toString().indexOf("Array") == -1) {
				  var _str = str;
			    }
			    else {
		...
		parseJSON : function (data) {
			...
			for(i in data.attributes) {
				if(i == "class") {
					...
					str += "' ";
/**********************************************************************
  Additions made by David Shorthouse
**********************************************************************/
					if(data.attributes[i].indexOf("synonym") >= 0) {
					  str += "title='Synonym' ";
				    }
					if(data.attributes[i].indexOf("vernacular") >= 0) {
					  str += "title='Vernacular' ";
				    }
				    if(data.attributes[i].indexOf("spelling") >= 0) {
					  str += "title='Alternative or Incorrect Spelling' ";
				    }
/*********************************************************************/
					cls = true;
				}
			...
			if(this.settings.languages.length) {
				...
				str += "<input type='checkbox' name='classification_tree[name_tags][" + vid + "][" + data.attributes['id'] + "]'></input>";
				if(data.icons) 
...
/**********************************************************
function adjusted by D. Shorthouse
**********************************************************/
				.listen("click", "input", function (event) { // WHEN CLICK IS ON THE CHECKBOX
					_this.toggle_checkbox.apply(_this, [event.target]);
					event.stopPropagation();
				})
/********************************************************/
...
/***********************************************************************
  Added by D. Shorthouse to add checked boxes from a callback
***********************************************************************/
			if(this.checked.length) {
				for(var i=0; i < this.checked.length; i++) {
					$('#' + this.checked[i]).children("input:checkbox:eq(0)").attr("checked",true);
				}
			}
/**********************************************************************/
...
/*****************************************************************
functions added by D. Shorthouse
*****************************************************************/
		toggle_checkbox : function(obj) {
			var obj = this.get_node(obj);
			if(obj.hasClass("closed")) {
				this.open_branch(obj);
			}
			if(obj.children("input:checkbox:eq(0)").is(":checked")) return this.add_checked(obj);
			if(obj.children("input:checkbox:eq(0):not(checked)")) return this.remove_checked(obj);
		},
		add_checked : function (obj) {
			var _this = this;
			var obj = _this.get_node(obj);
			obj.children("input:checkbox:eq(0)").removeClass("unchecked").addClass("checked");
			obj.find("li.closed").each(function(){
			  _this.open_branch(this);
		    });
			obj.children("ul").find("input:checkbox").attr("checked",true);
		},
		remove_checked : function (obj) {
			var obj = this.get_node(obj);
			obj.children("input:checkbox:eq(0)").removeClass("checked").addClass("unchecked");
			obj.children("ul").find("input:checkbox").removeAttr("checked");
		},
/***************************************************************/
...
/***************************************************************************************************
 Mod by David Shorthouse to work with Drupal callback using CleanURLs
 Original line:   $.getJSON(this.settings.data.url, { id : obj.attr("id") }, function (data, textStatus) {
 ***************************************************************************************************/
...
/***************************************************************************************
  Added by D. Shorthouse to auto-check the checkboxes and expand the children's children
***************************************************************************************/
						if(obj.children("input:checkbox:eq(0)").is(":checked")) {
							_this.add_checked(obj);
							obj.find("li.closed").each(function(){
							  _this.open_branch(this);
						    });
						}
